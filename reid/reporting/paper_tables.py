...
def _record_from_manifest(manifest_path: Path) -> dict[str, Any] | None:
    ...
    record = {
        ...
        "runtime_min": _finite_float(timings.get("total_run_min")),
        "run_id": run_id,
...
def _best_values(rows: Sequence[Mapping[str, Any]]) -> dict[str, float]:
    best: dict[str, float] = {}
...
def _row_csv(row: Mapping[str, Any]) -> dict[str, Any]:
    return {column: row.get(column) for column in TABLE_COLUMNS}
    ...
    *,
    ...
    header = [
        ...
        *source_comments,
        ...
        "% Requires graphicx; CVPR templates normally include it.",
        "\\resizebox{\\linewidth}{!}{%",
        "\\begin{tabular}{lll rrrrrrrr}",
        "\\toprule",
        'Method & Matcher & Checkpoint & $k$ & Top-1 (\\%) & Top-5 (\\%) & Top-10 (\\%) & Balanced Top-1 (\\%) & mAP (\\%) & mAP@k (\\%) & Runtime (min) \\\\',
        "\\midrule",
    ]
    body = []
    for row in rows:
        cells = [
            ...
            _latex_escape(row.get("checkpoint")),
    ...
    footer = [
        ...
        "}%",
        "\\end{table}",
        "% Percentages are source fractions multiplied by 100 for display.",
    ]
    ...
    *,
    ...
    outputs = {
        output_dir / f"{stem}_main.tex": render_latex(
            main_rows, animal=animal, table_name="main", candidate_k=main_candidate_k, generated_at=generated_at
        ),
        output_dir / f"{stem}_main.csv": render_csv(main_rows),
        output_dir / f"{stem}_ablation.tex": render_latex(
            ablation_rows, animal=animal, table_name="ablation", candidate_k=None, generated_at=generated_at
        ),
